...
def index():
    ...
    zip_codes = sorted(data['zip_codes'].unique())
...
# Handle unknow categories in the input data
...
app = Flask(__name__)
data = pd.read_csv('final_dataset.csv')
pipe = pickle.loads(open("RidgeModel.pkl", 'rb'))
...
def predict():
    bedrooms = request.form.get('beds')
    bathrooms = request.form.grt('baths')
    size = request.form.get('size')
    zipcode = request.form.get('zip_code')
    ...
    # Covert input data to numeric types
    input_data = input_data.astype({'beds': int, 'baths': float, 'size': float, 'zip_code': int})    #"astype" : Unknown word

    # Handle unknown categories in the inpute data
    for column in input_data.columns:
        unknown_categories = set(input_data[column]) - set(data[column].unique())
        if unknown_categories:
            print(f"Unknown categories in {column}: {unknown_categories}")
            # Handel unknown categories (e.g., replace with a default value)
            input_data[column] = input_data[column].replace(unknown_categories, data[column].mode()[0])
